...
public sealed class GigaChatHelper
{
        ...
        IEnumerable<string>? allergens)
    {
        ...
        var systemPrompt =
            ...
            "Ты создаёшь уникальное и разнообразное меню на каждый день. Блюда не должны повторяться. " +
            "Верни ТОЛЬКО валидный JSON без пояснений.";

        var userPrompt = $@"
Составь разнообразный план питания на {days} дней. БЛЮДА НЕ ДОЛЖНЫ ПОВТОРЯТЬСЯ в разные дни!
ЦЕЛЕВЫЕ ПОКАЗАТЕЛИ В СУТКИ: {targetCalories} ккал (Б:{targetProteins}г, Ж:{targetFats}г, У:{targetCarbs}г).

ПРАВИЛА БАЛАНСИРОВКИ:
1. Сумма КБЖУ 4 приемов (Breakfast, Lunch, Dinner, Snack) за каждый день должна максимально совпадать с целевыми показателями (погрешность не более 10%).
...
{(recipeTitles.Count > 0 ? "6. БАЗА РЕЦЕПТОВ: ЖЕЛАТЕЛЬНО использовать блюда из этого списка (выбирай подходящие по КБЖУ): " + string.Join(", ", recipeTitles) : "")}
